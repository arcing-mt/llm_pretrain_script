...
def calc_params_l2_norm(model, force_create_fp32_copy=False):
    ...
    if len(params_data) > 0:
        ...
        norm_2 = torch.zeros((1,), dtype=torch.float32, device='musa')
...
def calc_dtensor_params_l2_norm(params):
    ...
    total_norm_2 = torch.zeros((1,), dtype=torch.float32, device='musa')
    dummy_overflow_buf = torch.zeros((1,), dtype=torch.int, device='musa')
    for dtensor_spec, local_tensors in params_data.items():
        ...
        if len(local_tensors) == 0:
            norm = torch.zeros((1,), dtype=torch.float32, device='musa')
        else:
            norm, _ = multi_tensor_applier(
...
def report_memory(name):
    ...
    if len(MemMonitor.max_rank_ratio_list) > 0 :
