...
for host in ${ip_list[@]}; do
  echo "$host"
  ssh -f -n "$host" "bash -c 'cd \"${WORK_DIR}\"; WORLD_SIZE=${NNODES} RANK=${COUNT} MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MAIN_PROCESS_PORT} TRAINING_STEPS=${TRAINING_STEPS} LOG_NAME=${LOG_NAME} ENABLE_TENSORBOARD=${ENABLE_TENSORBOARD} MUSA_PRETRAIN_ENTRY=${MUSA_PRETRAIN_ENTRY:-} ENABLE_SEQUENCE_PARALLEL=${ENABLE_SEQUENCE_PARALLEL:-} bash ${DIST_TRAIN} > ${LOG_DIR}/log.${COUNT}.${host} 2>&1 &'"
